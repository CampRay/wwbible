...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// <summary>
        /// Get Daily product in Daily category
        /// </summary>
        /// <param name="categoryId">Category identifiers</param>
        /// <param name="storeId">Store identifier; 0 to load all records</param>
        /// <returns>Number of products</returns>
        public virtual Product GetDailyProductInCategory(int categoryId, int storeId = 0)
        {
            string dataStr = DateTime.Now.ToString("MMdd");            
            var query = _productRepository.Table;
            query = query.Where(p => !p.Deleted&& dataStr.Equals(p.ShortDescription));
        ...
        /// <param name="searchShortDescriptions"></param>
        /// <param name="searchDescriptions"></param>
        /// <param name="searchProductTags"></param>
        /// <param name="languageId"></param>
        /// <param name="orderBy"></param>
        /// <returns></returns>
        public virtual IPagedList<Product> SearchContent(            
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            IList<int> categoryIds = null,                                                                             
            int productTagId = 0,
            ...
            bool searchShortDescriptions = false,
            bool searchDescriptions = false,            
            bool searchProductTags = false,
            int languageId = 0,            
            ProductSortingEnum orderBy = ProductSortingEnum.Position)
        {
            ...
            if (!String.IsNullOrWhiteSpace(keywords))
            {
                query = from p in query
                        ...
                        from lp in p_lp.DefaultIfEmpty()
                        from pt in p.ProductTags.DefaultIfEmpty()
                        where (searchName&&p.Name.Contains(keywords)) ||
                              (searchShortDescriptions && p.ShortDescription.Contains(keywords)) ||
                              (searchDescriptions && p.FullDescription.Contains(keywords)) ||                              
                              //product tags (exact match)
                              (searchProductTags && pt.Name == keywords) ||
                              //localized values
                              (searchName&&searchLocalizedValue && lp.LanguageId == languageId && lp.LocaleKeyGroup == "Product" && lp.LocaleKey == "Name" && lp.LocaleValue.Contains(keywords)) ||
                              (searchShortDescriptions && searchLocalizedValue && lp.LanguageId == languageId && lp.LocaleKeyGroup == "Product" && lp.LocaleKey == "ShortDescription" && lp.LocaleValue.Contains(keywords)) ||
                              (searchDescriptions && searchLocalizedValue && lp.LanguageId == languageId && lp.LocaleKeyGroup == "Product" && lp.LocaleKey == "FullDescription" && lp.LocaleValue.Contains(keywords))
                        select p;
            }
