...
namespace Nop.Services.Messages
{
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        /// <summary>
        /// Sends a donate feedback message
        /// </summary>
        /// <param name="subscription">Newsletter subscription</param>
        /// <param name="languageId">Language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendDonateFeedbackMessage(NewsLetterSubscription subscription, int languageId)
        {
            if (subscription == null)
                throw new ArgumentNullException("Donate");

            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetActiveMessageTemplate("Donate.FeedbackMessage", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddDonateFeedbackTokens(tokens, subscription);
        ...
        #region Apply Lecture
        /// <summary>
        /// Sends a apply lectures activation message
        /// </summary>
        /// <param name="affiliate">affiliate</param>
        /// <param name="languageId">Language identifier</param>
        /// <returns>Queued email identifier</returns>
        public int SendApplyLectureActivationMessage(Affiliate affiliate, int languageId)
        {
            if (affiliate == null)
                throw new ArgumentNullException("JoinLectures");

            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetActiveMessageTemplate("JoinLectures.ActivationMessage", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddAffiliateTokens(tokens, affiliate);

            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);

            var toEmail = affiliate.Address.Email;
            var toName = affiliate.Address.FirstName;
            return SendNotification(messageTemplate, emailAccount,
                languageId, tokens,
                toEmail, toName);
        }
