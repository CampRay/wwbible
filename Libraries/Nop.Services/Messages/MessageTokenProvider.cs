...
namespace Nop.Services.Messages
{
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        public virtual void AddStoreTokens(IList<Token> tokens, Store store, EmailAccount emailAccount)
        {
            ...
            tokens.Add(new Token("Site.Name", store.GetLocalized(x => x.Name)));
            tokens.Add(new Token("Site.URL", store.Url, true));
            tokens.Add(new Token("Site.Email", emailAccount.Email));
            tokens.Add(new Token("Site.CompanyName", store.CompanyName));
            tokens.Add(new Token("Site.CompanyAddress", store.CompanyAddress));
            tokens.Add(new Token("Site.CompanyPhoneNumber", store.CompanyPhoneNumber));
            tokens.Add(new Token("Site.CompanyVat", store.CompanyVat));
        ...
        public virtual void AddDonateFeedbackTokens(IList<Token> tokens, NewsLetterSubscription subscription)
        {
            tokens.Add(new Token("Donate.Email", subscription.Email));
            tokens.Add(new Token("Donate.Cname", subscription.Cname));
            tokens.Add(new Token("Donate.Ename", subscription.Ename));

            //event notification
            _eventPublisher.EntityTokensAdded(subscription, tokens);
        }        

        public virtual void AddAffiliateTokens(IList<Token> tokens, Affiliate affiliate)
        {
            tokens.Add(new Token("JoinLectures.Email", affiliate.Address.Email));
            tokens.Add(new Token("JoinLectures.Name", affiliate.Address.FirstName));
            tokens.Add(new Token("JoinLectures.Phone", affiliate.Address.PhoneNumber));
            tokens.Add(new Token("JoinLectures.Churches", affiliate.Address.Company));
            var content = new StringBuilder();
            content.Append("<ul>");
            foreach (var item in affiliate.AffiliateProducts)
            {
                content.Append("<li><div>");
                content.Append("<p>"+ item.Name+"</p>");
                content.Append("<p>" + item.ShortDescription + "</p>");
                content.Append("<p>" + item.FullDescription + "</p>");                
                content.Append("</div></li>");
            }
            content.Append("</ul>");
            tokens.Add(new Token("JoinLectures.Lecture", content.ToString(),true));

            const string urlFormat = "{0}affiliate/activation/{1}";

            var activationUrl = String.Format(urlFormat, GetStoreUrl(), affiliate.Guid);
            tokens.Add(new Token("JoinLectures.ActivationUrl", activationUrl, true));            

            //event notification
            _eventPublisher.EntityTokensAdded(affiliate, tokens);
        }
        ...
        public virtual string[] GetListOfAllowedTokens()
        {
            ...
            var allowedTokens = new List<string>
            {
                ...
                "JoinLectures.Name",
                "JoinLectures.Phone",
                "JoinLectures.Churches",
                "JoinLectures.ActivationUrl"
            };
            allowedTokens.AddRange(additionTokens.AdditionTokens);
