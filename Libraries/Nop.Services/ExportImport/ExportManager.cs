...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        protected virtual string GetCategoryIds(Product product)
        {
            ...
            foreach (var pc in _categoryService.GetProductCategoriesByProductId(product.Id))
            {
                categoryIds += pc.Category.Id.ToString();
                categoryIds += ";";
            }
            return categoryIds;
        }
        ...
        public virtual byte[] ExportProductsToXlsx(IEnumerable<Product> products)
        {
            var properties = new[]
            {
                ...
                new PropertyByName<Product>("GroupName", p => p.ShortDescription),
                ...
                //    AllowBlank = true
                //},
                new PropertyByName<Product>("PageTemplate", p => p.ProductTemplateId, IgnoreExportPoductProperty(p => p.ProductTemplate))
                {
                ...
                new PropertyByName<Product>("HasPdfDownload", p => p.IsDownload, IgnoreExportPoductProperty(p => p.DownloadableProduct)),
                new PropertyByName<Product>("PdfDownloadId", p => p.DownloadId, IgnoreExportPoductProperty(p => p.DownloadableProduct)),
                ...
                //},
                new PropertyByName<Product>("HasVedioDownload", p => p.HasSampleDownload, IgnoreExportPoductProperty(p => p.DownloadableProduct)),
                new PropertyByName<Product>("VedioDownloadId", p => p.SampleDownloadId, IgnoreExportPoductProperty(p => p.DownloadableProduct)),
        ...
        /// <summary>
        /// Export affiliates list to XLSX
        /// </summary>
        /// <param name="stream">Stream</param>
        /// <param name="customers">Customers</param>
        public virtual byte[] ExportAffiliatesToXlsx(IList<Affiliate> affiliate)
        {

            //property array
            var properties = new[]
            {
                new PropertyByName<Affiliate>("Id", p => p.Id),
                new PropertyByName<Affiliate>("Name", p => p.Address.FirstName),
                new PropertyByName<Affiliate>("Phone Number", p => p.Address.PhoneNumber),
                new PropertyByName<Affiliate>("Email", p => p.Address.Email),
                new PropertyByName<Affiliate>("Churches", p => p.Address.Company),
                new PropertyByName<Affiliate>("Agree with receiving news", p => p.Active),
                new PropertyByName<Affiliate>("Been Confirmed", p => !p.Deleted),
                new PropertyByName<Affiliate>("Created Date", p => _dateTimeHelper.ConvertToUtcTime(p.Address.CreatedOnUtc, _dateTimeHelper.CurrentTimeZone)),
                new PropertyByName<Affiliate>("Lectures Ids", p => string.Join(",",p.AffiliateProducts.Select(o=>o.Id).ToArray())),
            };            

            return ExportToXlsx(properties, affiliate);
        }


        /// <summary>
        /// Export donate user list to XLSX
        /// </summary>
        /// <param name="stream">Stream</param>
        /// <param name="newsLetterSubscription">NewsLetterSubscription</param>
        public virtual byte[] ExportDonateUsersToXlsx(IList<NewsLetterSubscription> newsLetterSubscription)
        {
            //property array
            var properties = new[]
            {
                new PropertyByName<NewsLetterSubscription>("Id", p => p.Id),
                new PropertyByName<NewsLetterSubscription>("Chinese Name", p => p.Cname),
                new PropertyByName<NewsLetterSubscription>("English Name", p => p.Ename),
                new PropertyByName<NewsLetterSubscription>("Phone Number", p => p.Phone),
                new PropertyByName<NewsLetterSubscription>("Email", p => p.Email),
                new PropertyByName<NewsLetterSubscription>("Gender", p => (p.Gender?"男":"女")),
                new PropertyByName<NewsLetterSubscription>("Address", p => p.Address),
                new PropertyByName<NewsLetterSubscription>("Churches", p => p.Church),
                new PropertyByName<NewsLetterSubscription>("Amount", p => p.Amount),
                new PropertyByName<NewsLetterSubscription>("Agree with receiving news", p => p.Active),
                new PropertyByName<NewsLetterSubscription>("願意參與義工工作", p => p.Participation),
                new PropertyByName<NewsLetterSubscription>("願意按月奉獻支持", p => p.Monthly),
                new PropertyByName<NewsLetterSubscription>("寄劃線支票", p => p.MailCheck),
                new PropertyByName<NewsLetterSubscription>("存入銀行戶口", p => p.Deposit),
                new PropertyByName<NewsLetterSubscription>("自動轉帳", p => p.Virement),
            };
            return ExportToXlsx(properties, newsLetterSubscription);
        }
