...
namespace Nop.Core.Html
{
    ...
    public partial class HtmlHelper
    {
        ...
        /// <summary>
        /// 获取指定开始结束节点中的数据, start = <tbody>, end = </tbody>
        /// </summary>
        public static string subString(string data, string start, string end)
        {
            try
            {
                int i1 = data.IndexOf(start,StringComparison.OrdinalIgnoreCase) + start.Length, i2 = data.IndexOf(end, i1, StringComparison.OrdinalIgnoreCase);
        ...
        /// <summary>
        /// 获取指定开始结束节点中的数据, 从开始寻找start = <tbody>, 从结尾寻找end = </tbody>
        /// </summary>
        public static string subStringE(string data, string start, string end)
        {
            try
            {
                int i1 = data.IndexOf(start, StringComparison.OrdinalIgnoreCase) + start.Length, i2 = data.LastIndexOf(end, StringComparison.OrdinalIgnoreCase);
                string tmp = data.Substring(i1, i2 - i1);
        ...
        public static List<string> getNodes(string data, string nodeName)
        {
            ...
            try
            {
                ...
                while (pos != -1)
                {
                    ...
                    string end = "/" + nodeName + ">";
                    pos = data.IndexOf(end, StringComparison.OrdinalIgnoreCase) + end.Length;// 记录当前解析的最后位置
        ...
        {
            string tmp = node;
            foreach (string name in nodeName) if (tmp.Contains("<" + name)) tmp = getNode(tmp, name);
            return tmp;
        }
