...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
            ...
            bool loadSubCategories = true,IList<Category> allCategories = null)
        {
            ...
            foreach (var category in categories)
            {
                var categoryModel = new CategorySimpleModel
                {
                    ...
                    IncludeInYiJinMenu=category.IsYinJinMenu,
                    IncludeInHongDaoMenu=category.IsHongDaoMenu
                };
        ...
        /// <summary>
        /// Prepare book category (simple) models
        /// </summary>
        /// <param name="rootCategoryId">Root category identifier</param>
        /// <param name="loadSubCategories">A value indicating whether subcategories should be loaded</param>
        /// <param name="allCategories">All available categories; pass null to load them internally</param>
        /// <returns>Category models</returns>
        [NonAction]
        protected virtual IList<CategorySimpleModel> PrepareBookCategoryModels(int rootCategoryId,IList<Category> allCategories = null)
        {
            var result = new List<CategorySimpleModel>();
            ...
            foreach (var category in categories)
            {                               
                var subCategories = PrepareBookCategoryModels(category.Id,  allCategories);
        ...
        protected virtual IEnumerable<ProductOverviewModel> PrepareProductOverviewModels(IEnumerable<Product> products,
            bool preparePriceModel = false, bool preparePictureModel = false,
        ...
        [NonAction]
        protected virtual IList<Nop.Web.Models.Catalog.SearchBookModel.VerseModel> PrepareVersesOverviewModels(IEnumerable<Product> products, string q)
        {
            var models = new List<Nop.Web.Models.Catalog.SearchBookModel.VerseModel>();
            foreach (var product in products)
            {                
                //string regex = "<span class=\"v-text\"></span>";
                string regexTr = @"(?is)<tr[^>]*>(?:(?!</tr>).)*</tr>";
                string regexTd = @"(?is)<td[^>]*>(?:(?!</td>).)*</td>";
                Regex reTr = new Regex(regexTr, RegexOptions.IgnoreCase);
                Regex reTd = new Regex(regexTd, RegexOptions.IgnoreCase);                
                MatchCollection matchesTr = reTr.Matches(product.FullDescription);

                //List<string> trList=Nop.Core.Html.HtmlHelper.getNodes(product.FullDescription, "tr");
                foreach (Match match in matchesTr)
                {
                    if (match.Value.Contains(q))
                    {
                        ...
                        MatchCollection matchesTd = reTd.Matches(match.Value);
                        foreach (Match matchTd in matchesTd)
                        {
                            if (matchTd.Value.IndexOf("class=\"v-heading\"") >0)
                            {
                                isHeading = true;
                                break;
                            }
                            if (matchTd.Value.IndexOf("class=\"v-td1\"") > 0)
                            {
                                verseNo = Nop.Core.Html.HtmlHelper.getNode(matchTd.Value, "td");
                            }
                            if (matchTd.Value.IndexOf("class=\"v-td2\"") > 0)
                            {
                                verseText = Nop.Core.Html.HtmlHelper.getNode(matchTd.Value, "span");
                            }
                        }
                        if (!isHeading)
                        {
                            var model = new Nop.Web.Models.Catalog.SearchBookModel.VerseModel
                            {
                                BookName = product.ProductCategories.FirstOrDefault().Category.Name,
                                CategoryId = product.ProductCategories.FirstOrDefault().Category.Id,
                                Chapter = product.Name,
                                VerseNo = verseNo,
                                VerseText = verseText,
                                BookUrl = Url.RouteUrl("Product", new { SeName = product.GetSeName() })
                            };
                            models.Add(model);
                        }
                    }
        ...
        [ChildActionOnly]
        public ActionResult BookNavigation(int currentCategoryId)
        {
            //get active category
            int activeCategoryId = 0;
            if (currentCategoryId > 0)
            ...
            string cacheKey = string.Format(ModelCacheEventConsumer.BOOK_NAVIGATION_MODEL_KEY,
                _workContext.WorkingLanguage.Id,
                _storeContext.CurrentStore.Id,currentCategoryId);
            var cachedModel = _cacheManager.Get(cacheKey, () => PrepareCategorySimpleModels(currentCategoryId).ToList());
        ...
        [HttpPost]
        public ActionResult BookCategoryList(int categoryId)
        {            
            string cacheKey = string.Format(ModelCacheEventConsumer.BOOK_CATEGORY_MODEL_KEY,
                _workContext.WorkingLanguage.Id,
                _storeContext.CurrentStore.Id,categoryId);
            var cachedModel = _cacheManager.Get(cacheKey, () => PrepareBookCategoryModels(categoryId).ToList());           
        ...
        public ActionResult CategoryProducts(int categoryId)
        {
            ...
            //products
            var products = _productService.SearchProducts(categoryIds: categoryIds);
            var productsModel = products.Select(p =>
              {
                  var productModel = new
                  {
                      Id = p.Id,
                      Name = p.GetLocalized(y => y.Name),
                      Url = Url.RouteUrl("Product", new { SeName = p.GetSeName() })
                  };
                  return productModel;
              }).ToList();
        ...
        [ChildActionOnly]
        public ActionResult CategoryProductsNavigation(int currentCategoryId, int currentProductId)
        {
            //get active category
            int activeCategoryId = 0;
            if (currentCategoryId > 0)
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        [ValidateInput(false)]
        public ActionResult SearchBook(string searchTerms, int categoryId)
        {
            ...
            if (!string.IsNullOrEmpty(searchTerms))
            {
                ...
                //products
                IPagedList<Product> products = _productService.SearchContent(
                    categoryIds: categoryIds,                                        
                    keywords: searchTerms,
                    searchName:false,
                    searchShortDescriptions:false,
                ...
                var category = _categoryService.GetCategoryById(categoryId);
                model.q = searchTerms;
                model.BibleName=category.Name;
                model.Verses = PrepareVersesOverviewModels(products,searchTerms);                
                model.NoResults = !model.Verses.Any();
            }
