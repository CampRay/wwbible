...
namespace Nop.Web.Controllers
{
    public partial class DownloadController : BasePublicController
    {
        ...
        public ActionResult GetFileDownload(int downloadId,bool isDownload=false)
        {
            var download = _downloadService.GetDownloadById(downloadId);

            if (download == null)
                return Content("沒有找到相關記錄");

            if (download.UseDownloadUrl)
                return new RedirectResult(download.DownloadUrl);

            if (download.DownloadBinary == null)
                return Content("沒有數據內容.");
            ...
            if (isDownload)
            {
                return new FileContentResult(download.DownloadBinary, MimeTypes.ApplicationOctetStream) { FileDownloadName = fileName + download.Extension };
            }
            else
            {
