...
namespace Nop.Admin.Controllers
{
    public partial class DownloadController : BaseAdminController
    {
        ...
        public ActionResult AsyncUpload()
        {
            ...
            var directory = "~/Content/Vedios/uploaded/";
            var newFileName = DateTime.Now.Ticks + fileExtension;
            var filePath = Path.Combine(CommonHelper.MapPath(directory), newFileName);            

            byte[] fileBinary = null;
            bool useDownloadUrl = false;
            string downloadUrl = "";
            try
            {
                if (stream.Length > 32000000)
                {
                    ...
                    fs.Close();
                    useDownloadUrl = true;
                    downloadUrl = Url.Content("~/Content/Vedios/uploaded/" + newFileName);
                }
                else
                {
