	...
	public partial class NewsLetterSubscriptionController : BaseAdminController
	{
		...
		public ActionResult SubscriptionList(DataSourceRequest command, NewsLetterSubscriptionListModel model)
        {
            ...
            var newsletterSubscriptions = _newsLetterSubscriptionService.GetAllNewsLetterSubscriptions(model.SearchEmail,
                startDateValue, endDateValue, 0, isActive, 0,
        ...
        public ActionResult SubscriptionUpdate([Bind(Exclude = "CreatedOn")] NewsLetterSubscriptionModel model)
        {
            ...
            subscription.Participation = model.Participation;
            subscription.Monthly = model.Monthly;
            subscription.Amount = model.Amount;
            subscription.Cname = model.Cname;
            subscription.Gender = model.Gender;
            subscription.Phone = model.Phone;
            subscription.Address = model.Address;
        ...
        public ActionResult ExportExcelAll(NewsLetterSubscriptionListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageNewsletterSubscribers))
                return AccessDeniedView();
            ...
            var subscriptions = _newsLetterSubscriptionService.GetAllNewsLetterSubscriptions(model.SearchEmail,
                startDateValue, endDateValue, 0, isActive, 0);
            try {           
                byte[] bytes = _exportManager.ExportDonateUsersToXlsx(subscriptions);            
                return File(bytes, MimeTypes.TextXlsx, "DonateUsers.xlsx");
            }
            catch (Exception exc)
            {
